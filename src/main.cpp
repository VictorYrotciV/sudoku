...
int main(int argc, char const *argv[])
{
    ...
    for(int i=1;i<argc;i++)
    {
        if(!strcmp(argv[i], "-c"))
        {
            ...
            generateAndSaveSudokuFinalBoards(final_board_num, finalboards_file_path);
        }
    ...
    else if(is_n)
    {
        ...
        generateAndSaveSudokuGameBoards(game_num, blank_lower_num, blank_upper_num, game_level, games_file_path);
    }
