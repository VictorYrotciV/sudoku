...
bool isRight(const std::vector<std::vector<int>> &grid,
    ...
    for (int row = 0; row < 3; row++) {
        for (int col = 0; col < 3; col++) {
            if (grid[row + mrow / 3 * 3][col + mcol / 3 * 3] == num
            && (mcol != col + mcol / 3 * 3 || mrow != row + mrow / 3 * 3)) {
                return false;
            }
        }
    }
...
TEST(TestCases, level_case_1) {
    ...
    for (int i = 0; i < game_num; i++) {
        ...
        std::vector<std::vector<int>> sudoku
            = genGameBoard(final_boards[index], r, game_level);
...
int main(int argc, char **argv) {
    genAndSaveFinalBoards(100, DEFAULT_FINAL_FILE_PATH);
